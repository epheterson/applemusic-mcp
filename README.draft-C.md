...
Apple Music for your AI assistant. Manage playlists, add music, control playback, and browse your library, from Claude, Cursor, Cline, Windsurf, or any [MCP client](https://modelcontextprotocol.io/clients).

Runs on macOS, Windows, and Linux. Sign in once, and it works the same everywhere.
...
Three engines, picked automatically by the `mode` preference (`auto` is the default; `native` pins macOS Music.app, `api` pins the cross-platform API + web player). A `✓` means supported, `✗` means not possible on that engine, `—` means not applicable there.
...
Everything in the **API** column runs anywhere with no browser and no Music app. Browser playback and the queue need Google Chrome and a desktop session. Full per-capability reasoning: [docs/CAPABILITIES.md](docs/CAPABILITIES.md).
