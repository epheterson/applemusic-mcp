...
- **A refused rollback claimed the cleanup had happened.** When creating a playlist inside a folder fails at the move step, the orphan is deleted — but that delete can now legitimately refuse (duplicate names), and its result was discarded while the message still said "Playlist was removed." It now reports what actually happened.
- **Disambiguating a broad term could time out instead of refusing.** The exact-title check read `name of` per match, one Apple Event each: 8478 matches took ~10s, so a broad term hit the 30s `osascript` deadline and reported a timeout rather than the candidate list — failing in exactly the high-ambiguity case the guard exists for. It is now a single query: **10.3s → 0.6s**.
...
- **Listing playlist tracks was ~10x slower than it should be, and always reported genre and year as empty.** The bulk fast path read each property off an intermediate `set allTracks to tracks of targetPlaylist`. That materializes a plain AppleScript list, and a property of a plain list does not distribute, so Music answers -1728 and names every track of the playlist in the error message. The fast path therefore failed on *every* call, and every listing was quietly served by the per-track fallback, which skips genre and year for speed. Reading the range off the playlist itself (`name of tracks 1 thru N of targetPlaylist`) restores it, keeping the bound-before-fetch from [#53](https://github.com/epheterson/applemusic-mcp/pull/53) by [@nkcmr](https://github.com/nkcmr), which found the unbounded property read. On a 12,457-track playlist at the default limit of 500: **7.3s to 1.2s, with genre and year populated** (a track with no year still reads as empty rather than as year 0).
...
- **`limit` is validated and clamped before it reaches AppleScript.** It was interpolated into the script as-is, where a bound below 1 is an error rather than an empty result, and a negative bound counts back from the *end* of the playlist. A `limit` below 1 now yields an empty list, and a non-numeric `limit` is rejected with a message instead of reaching `osascript`.
...
- **Genre search with zero matches no longer reports "not available."** On a
  macOS host with a developer token configured, a genre search that genuinely
  matched no tracks fell through to the API-fallback guard and wrongly claimed
  genre filtering "isn't available through the Apple Music API." It now returns
  a plain "No tracks found" — genre is local-only and never cascades to the API.

### Internal

- **Test suite no longer leaks `_TEST_*` folders into the user's library.** The
  session cleanup fixture swept only `user playlist`; Music's `folder playlist`
  is a separate collection, so every test folder leaked. The sweep now covers
  folders and playlists and runs as both pre-clean and teardown.
