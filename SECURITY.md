...
- **Tokens** (the Apple Developer token, your `media-user-token`, and the harvested web-player token) are stored in the **OS keychain** when a backend is available — macOS Keychain, Windows Credential Locker, or Linux Secret Service. When no backend exists (e.g. a headless server), they fall back to a `0600` JSON file under `~/.config/applemusic-mcp/`. A secret lives in exactly one place at a time.
